...
#5.6.2: Going to try to implement the rule generator.
...
class Rules(object):
    ...
    '''
    def __init__(self, rule_number, number_of_neighbors=3, values=[1, 0]): #test
        self.rule = generate_rule(rule_number, number_of_neighbors=3, values=[1, 0])
    ...
    @staticmethod #Can I somehow make something like this a static method. Update:No, because I have to call the class anyway (I think, but maybe not?)
    def generated_rule(above): #test
        '''A static method for the generated rule, which takes a codon and
        outputs the currect value.'''
        return self.rule[above]
...
##        return [self.rule(i) for i in _above(previous_row)] #Original that returns list.
    ...
    def create_grid(self, n, grid_width):
        ...
        width = grid_width
        grid = []
##        grid = Grid(width, height) #Original
...
class PygameRenderer(object):
    ...
    def render(self, grid):
        ...
        pygame.display.flip()
        print ('grid =', grid) #test
##        print (self.grid.seed)

##        self.grid = grid #Original
##        self.surface.fill(self.background)
...
##def test_grid(n = 5, grid_width = 200, pixel_size = 16, rule=Rules.rule150): #Original
...
def test_grid(n = 5, grid_width = 200, pixel_size = 16, rule=Rules.rule150): #test
    '''Creates a normal grid.'''
    rule = Rules(30)
    grid = Generator(rule=rule)(n, grid_width)
    r = PygameRenderer(n, grid_width, pixel_size)
...
if __name__ == '__main__':
    ...
    test_grid(30, 30, 4, 30) #The last arg is to generate rule 30.
