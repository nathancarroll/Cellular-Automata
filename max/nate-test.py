...
def test_grid(n, grid_width, pixel_size, rule,
    ...
    grid = Propagator(rule=rule_dict)(n, grid_width, randomness, values, number_of_neighbors)
    r = PygameRenderer(n, grid_width, pixel_size, colors, rule, values, number_of_neighbors)
    r.render(grid)
    while True:
        r.refresh()

# Run the test_grid function when the module is run as a program, not on import or require
if __name__ == '__main__':
    test_grid(1000, 1400, 1, 346113254264, 0, [2, 1, 0], 3, {'2': (0, 0, 255), '1': (0, 0, 0), '0': (255, 255, 255)})