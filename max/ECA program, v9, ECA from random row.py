#Version 8:
#-Goal: Add a binary input option.
...
def generate_rule(rule_number, number_of_neighbors=3, values=[1, 0], rule_num_base=10):
    ...
    `rule_num_base` allows you to set the base equal to the number of values, thus
        avoiding the need for the program to turn the rule number into the appropriate base.
        This allows you to more easily edit the rules_dict by playing with individual numbers in the input.
        -Default is 10. Set it to the number of values if you want to edit rules more finely.'''
    rules_dict = {}
    ...
    if rule_num_base == 10:
        if rule_number >= ((len(values)) ** (len(values) ** number_of_neighbors)):
            ...
            sys.exit()
    elif rule_num_base == len(values):
        if len(str(rule_number)) > ((len(values)) ** number_of_neighbors):        
            print ('The rule_number entered is too large to use given the rule values and number_of_neighbors you entered.')
            print ('The upper limit of digits for base ' + str(rule_num_base) + ' =', (len(values) ** number_of_neighbors))
            sys.exit()
    ...
    xary = '' #To be the string of a number in the base of the number of values.
    if rule_num_base == 10:
        n = rule_number
        base = len(values)
        while n > 1:
        ...
        xary = str(rule_number)

    else:
        print ('rule_num_base should equal 10 or the number of values.')
        sys.exit()

    while len(xary) < len(codons): #Adds extra zeros to the front of the number so all codons are matched to a value.
        xary = '0' + xary

    #Creates a decimal for the rule_number if it was entered in a base other than 10.
    #START HERE
    ...
    elif rule_num_base == len(values): #Just need to print the decimal of the rule_number.
        #print ('rule_number in base equal to the number of cell values =', xary)
...
def test_grid(n, grid_width, pixel_size, rule, rule_num_base,
              randomness=0, values=[1,0], number_of_neighbors=3,
              colors={'1': (0, 0, 0), '0': (255, 255, 255)}):
    '''Creates a normal grid.'''
    rule_dict = generate_rule(rule, number_of_neighbors, values, rule_num_base)
...
#13462 6561 19683 59049
    ...
    #For more complex CA.
    test_grid(1000, 1400, 1, 121102000210220110220002222, 3, 0, [2, 1, 0], 3,
