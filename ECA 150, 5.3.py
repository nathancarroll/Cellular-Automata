...
#Version 5.3:
#-Got rid of the centering mechanisms that screwed up indexing.
#-Changed test code from: grid = Generator(rule=rule)(n, grid_width)
                    #to: grid = Generator(rule=rule)(n)(grid_width)
                    #which solved something. I'm not sure that's a sturdy solution.
                    #I switched it back as it worked better then.
#-Also maybe some other stuff I'm forgetting.
#-There's still some kind of problems with the manually inputted arguments and
    #the default ones, such that I have to change it in multiple places for it
    #to work correctly whenever I change the parameter values.
...
class Generator(object):
    ...
    def random_first_row(self, grid_width):
    ...
    def generate(self, n=None, grid_width=1000):
##    def generate(self, n=None):
    ...
    def create_grid(self, n, grid_width=1000):
...
def test_grid(n = 5, grid_width = 200, pixel_size = 16, rule=Rules.rule150):
    ...
    grid = Generator(rule=rule)(n) #test
...
if __name__ == '__main__':
    ...
    test_grid(700, 1000, 1, Rules.rule110)
